...
import { mapNl as t, aiNl } from "@/lib/nl";
...
import { RecordResearch } from "@/components/ai/record-research";

export function MapRecordDetail({ entry, onClose }: { entry: MapEntry; onClose: () => void }) {
  const { record } = entry;
  const detail = useQuery({ queryKey: ["map-record", record.ondernemingsnr], queryFn: async ({ signal }) => mapDetailSchema.parse(await (await apiFetch(`/api/record/${encodeURIComponent(record.ondernemingsnr)}`, { signal: AbortSignal.any([signal, AbortSignal.timeout(15_000)]) })).json()) });
  ...
  return <div className="min-h-0 flex-1 overflow-y-auto">
    ...
    <div className="space-y-5 p-4">
      ...
      {detail.isPending ? <p role="status" className="text-xs text-muted-foreground">{t.loading}</p> : detail.isError ? <div role="alert" className="space-y-2"><p className="text-xs text-destructive">{t.detailError}</p><Button variant="outline" size="sm" onClick={() => void detail.refetch()}>{t.retry}</Button></div> : <>
        ...
        {!!detail.data?.correcties.length && <section className="space-y-3 rounded-lg border border-primary/25 bg-primary/5 p-3"><h4 className="text-sm font-semibold">{aiNl.acceptedCorrections}</h4><p className="text-xs leading-relaxed text-muted-foreground">{aiNl.correctionsNote}</p><dl className="space-y-3">{detail.data.correcties.map(correction => <div key={correction.id}><dt className="text-xs text-muted-foreground">{Object.hasOwn(aiNl.fields, correction.field) ? aiNl.fields[correction.field as keyof typeof aiNl.fields] : aiNl.correctionFieldUnknown}</dt><dd className="mt-1 break-words text-sm font-medium">{correction.value}</dd><dd className="mt-1 text-[11px] text-muted-foreground">{aiNl.confirmed} · {displayObservationDate(correction.createdAt.slice(0, 10))}</dd></div>)}</dl></section>}
      </>}
      <RecordResearch key={record.ondernemingsnr} nr={record.ondernemingsnr} name={record.commercieleNaam || record.naam || t.noName} />
