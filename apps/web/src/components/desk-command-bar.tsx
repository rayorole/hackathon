  ...
  return (
    <>
      ...
      >
        ...
        <Kbd className="ml-auto hidden shrink-0 text-[10px] sm:inline-flex">
          Ctrl / ⌘ K
        </Kbd>
      </Button>
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          ...
          className="desk-command-dialog top-[15%] translate-y-0 gap-0 overflow-hidden p-0 sm:max-w-xl"
        >
          ...
          <Command className="desk-command">
            ...
            <CommandList className="max-h-[min(50vh,360px)]">
              ...
              <CommandGroup heading={c.search}>
                ...
                {query.trim() && (
                  <CommandItem
                    value={`${c.search} ${query}`}
                    onSelect={() => run(() => onSearch(query.trim()))}
                  >
                    <ArrowUpRight />
                    <span>
                      {c.searchFor} “{query.trim()}”
                    </span>
                  </CommandItem>
                )}
                {streets.map((street) => (
              ...
              <CommandGroup heading={c.navigation}>
                {(Object.keys(deskRoutes) as DeskScreen[])
                  .filter((screen) => screen !== "states" && screen !== "map")
                  .map((screen) => (
                    <CommandItem
                      key={screen}
                      value={t.nav[screen]}
                      onSelect={() => run(() => onNavigate(screen))}
                    >
                      <ArrowUpRight />
                      {t.nav[screen]}
                    </CommandItem>
              ...
              <CommandGroup heading={c.records}>
                {records.map((record) => (
                  <CommandItem
                    key={record.proposal?.id ?? record.id}
                    value={`${record.naam} ${record.adres} ${record.ondNr} ${record.vestNr}`}
                    onSelect={() => run(() => onOpenRecord(record))}
                  >
                    <Building2 className="text-muted-foreground" />
                    <span className="min-w-0">
                      <span className="block truncate">{record.naam}</span>
                      <span className="block truncate text-xs font-normal text-muted-foreground">
                        {record.adres} · {record.soort}
                      </span>
                    </span>
                  </CommandItem>
                ))}
