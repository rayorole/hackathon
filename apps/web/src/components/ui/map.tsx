/* eslint-disable react-hooks/refs, react-hooks/set-state-in-effect -- Vendored mapcn imperative MapLibre bridge; retain upstream lifecycle synchronization. */
// Source: https://mapcn.vercel.app/r/map.json (2026-09-16). MIT license: map.LICENSE. Local changes: Dutch labels and data-driven point colors.
"use client";
...
function MapControls({
  ...
  return (
    ...
    >
      {showZoom && (
        <ControlGroup>
          <ControlButton onClick={handleZoomIn} label={mapNl.zoomIn}>
            <Plus className="size-4" />
          </ControlButton>
          <ControlButton onClick={handleZoomOut} label={mapNl.zoomOut}>
      ...
      {showFullscreen && (
        <ControlGroup>
          <ControlButton onClick={handleFullscreen} label={mapNl.fullscreen}>
...
function CompassButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <ControlButton onClick={onClick} label={mapNl.north}>
      <svg
        ref={compassRef}
        viewBox="0 0 24 24"
        className="size-5"
